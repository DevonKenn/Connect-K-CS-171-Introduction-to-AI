...
class StudentAI():
	...
	def maxV(self,model,depth,alpha,beta,best =[],prior_moves = [],moves =[]):
		if(self.deadline-(int(round(time.time() * 1000 )) - self.start_time) <1000):
			return float("-inf"),prior_moves,moves

		if(depth <=0 or model.winner()):
			return self.heuristic(model,depth),prior_moves,moves
		val = float("-inf");
		best_move = prior_moves
		nm = []
		move = ()
		if(len(best)>0):
	...
	def checkRight(self,model,move,player,second = False):
		...
		if((x-1<0 or model.pieces[x-1][y] != player) ):
			if(x-1>=0 and model.pieces[x-1][y] ==0):
				head = True
			count =1
			back_count = 0
			...
			if(second):
				return count
			while( x-back_count-1 >=0 and (model.pieces[x-back_count-1][y] ==model.pieces[x][y] or model.pieces[x-back_count-1][y] ==0) ):
				back_count +=1
			if(x+count < model.width and model.pieces[x+count][y] ==0 ):#else tail is false, it ends at an enemy or wall
				tail =True
				if(count == model.k_length -1 and head):#if head and tail then you always win!
					return (10 ** (model.k_length + 2))
		...
		superCount = count + secondCount
		return self.kCalcs(model,superCount)
	...
	def checkDiagUpRight(self,model,move,player,second = False):
		...
		if((x-1<0 or y-1<0 or model.pieces[x-1][y-1] != player) ):#and (x-2<0 or y-2<0 or model.pieces[x-2][y-2]!=player)):
			if(x-1>=0 and y-1>=0 and model.pieces[x-1][y-1] ==0):
				head = True
			count =1
			back_count = 0
			...
			if(second):
				return count
			while(y-back_count-1 >=0  and x+back_count+1 <model.height and (model.pieces[x-back_count-1][y-back_count-1] ==model.pieces[x][y] or model.pieces[x-back_count-1][y-back_count-1] ==0) ):
				back_count +=1
			if(y+count <model.height and x+count < model.width and model.pieces[x+count][y+count] ==0 ):#else tail is false, it ends at an enemy or wall
				tail =True
				if(count == model.k_length -1 and head):#if head and tail then you always win!
					return (10 ** (model.k_length + 2))
			if(count + back_count  < model.k_length and not tail):
	...
	def checkDiagDownRight(self,model,move,player,second = False):
		...
		head = False
		tail = False
		if((x-1<0 or y+1>=model.height or model.pieces[x-1][y+1] != player)):# and (y-2>=0 or x+2<=model.width or model.pieces[x-2][y-2] !=player)):
			if(x-1>=0 and y+1<model.height and model.pieces[x-1][y+1] ==0):
				head = True
			count =1
			back_count = 0
			...
			if(second):
				return count
			while(y+back_count+1 <model.height  and x-back_count-1 >=0 and (model.pieces[x-back_count-1][y+back_count+1] ==model.pieces[x][y] or model.pieces[x-back_count-1][y+back_count+1] ==0) ):
				back_count +=1

			if(y-count >=0 and x+count < model.width and model.pieces[x+count][y-count] ==0 ):#else tail is false, it ends at an enemy or wall
				tail =True
				if(count == model.k_length -1 and head):#if head and tail then you always win!
					return (10 ** (model.k_length + 2))
			if(count + back_count  < model.k_length and not tail):
	...
	def kCalcs(self,model,superCount):
		if(superCount < model.k_length):
			return superCount * (10 ** (model.k_length-(model.k_length-superCount) ) )
		else:
			return superCount * (10 ** (model.k_length+4))
	...
	def miniMax(self,model,depth,deadline):
	...
	def make_move(self,model, deadline):
		...
		return self.miniMax(self.model,9,deadline)
